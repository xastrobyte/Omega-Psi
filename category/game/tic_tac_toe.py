...
class TicTacToe:
    ...
    def get_message(self):
        """Returns the message sent during gameplay
        """
        return self._message
    
    def set_message(self, message):
        """Sets the message message sent during gameplay
        """
        self._message = message

    def get_challenger(self):
        """Returns the  of the Tic-Tac-Toe game.
        """
    ...
    def make_move(self, spot):
        ...
        # Check if spot was taken already
        if self._board[spot] != None:
            return TicTacToe.SPOT_TAKEN
        ...
        # Check if going against actual opponent
            ...
            self._board[spot] = self.is_challenger_move()
            self.next_turn()
        
        # Check for a winner / draw
        spotsLeft = self.count_spots_left()
        winnerCheck = self.check_for_winner()
    ...
    def get_ai_move(self):
        ...
        if self._difficulty in ["hard"]:
            ...
            leftDiag = self.should_ai_go([self._board[0], self._board[4], self._board[8]])
            rightDiag = self.should_ai_go([self._board[2], self._board[4], self._board[6]])
            if leftDiag["boolean"]:
        ...
        if self._difficulty in ["medium", "hard"]:
            ...
            firstColumn = self.should_ai_go([self._board[0], self._board[3], self._board[6]])
            secondColumn = self.should_ai_go([self._board[1], self._board[4], self._board[7]])
            thirdColumn = self.should_ai_go([self._board[2], self._board[5], self._board[8]])
        ...
        if self._difficulty in ["easy", "medium", "hard"]:
            ...
            firstRow = self.should_ai_go(self._board[ : 3])
            secondRow = self.should_ai_go(self._board[3 : 6])
            thirdRow = self.should_ai_go(self._board[6 : ])
            if firstRow["boolean"]:
        ...
        while self._board[location] != None and self.count_spots_left() > 0:
            location = randint(0, 8)
        
        self._board[location] = False
    
    def should_ai_go(self, line):
