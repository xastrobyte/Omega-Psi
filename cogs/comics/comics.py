...
async def xkcd_comic_refresh(ctx, comic_number):
    """Grabs the comic at the specified number and returns the title, description, and url for
    an embed

    :param ctx: The context of where the message was sent
    :param comic_number: The number of the comic to load and show
    """
...
class Comics(Cog, name="comics"):
    ...
    async def marvel(self, ctx):
        """Allows a user to search a Marvel character, comic, or comic series
    ...
    async def marvel_character(self, ctx, *, character=None):
        """Allows a user to search a Marvel character
    ...
    async def marvel_series(self, ctx, *, series=None):
        """Allows a user to search a Marvel comic series
    ...
    async def marvel_comic(self, ctx, *, comic=None):
        """Allows a user to search for a specific Marvel comic

        :param ctx: The context of where the message was sent
        :param comic: The comic to search for
        """
    ...
    async def xkcd(self, ctx):
        """Allows a user to search for or retrieve a random XKCD comic
        ...
        if not ctx.invoked_subcommand:
            ...
            await process_scrolling(ctx, self.bot, refresh_function=xkcd_comic_refresh, min_page=1,
                                    max_page=recent["num"], current_page=comic["num"])
    ...
    async def xkcd_recent(self, ctx):
        """Gives a user the most recent XKCD comic
        ...
        # Process the scrolling
        await process_scrolling(ctx, self.bot, refresh_function=xkcd_comic_refresh, min_page=1,
                                max_page=recent["num"], current_page=recent["num"])
    ...
    async def xkcd_find(self, ctx, comic_number=None):
        """Gives a user the specified XKCD comic based off the number

        :param ctx: The context of where the message was sent
        :param comic_number: The number of the comic to get
        """
    ...
    async def strange_planet(self, ctx):
        """Allows a user to retrieve a random Strange Planet comic
    ...
    async def strange_planet_recent(self, ctx):
        """Gives a user the most recent Strange Planet comic
    ...
    async def strange_planet_search(self, ctx):
        """Allows a user to search for a specific comic or reaction from the Strange Planet
        comic series
        ...
        if not ctx.invoked_subcommand:
            await ctx.send(
                embed=get_error_message(
                    "You must specify whether you want to look for comics or reactions. "
                    "Try `{}help strangeplanet search`".format(
                        await database.guilds.get_prefix(ctx.guild) if ctx.guild else ""
                    )
                )
            )
    ...
    async def strange_planet_search_comics(self, ctx, *, keywords=None):
        """Allows a user to search for a comic from the Strange Planet
        comic series
        
        :param ctx: The context of where the message was sent
        :param keywords: Search keywords to use to find a comic
        """
    ...
    async def strange_planet_search_reactions(self, ctx, *, keywords=None):
        """Allows a user to search for a reaction from the Strange Planet
        comic series
        
        :param ctx: The context of where the message was sent
        :param keywords: Search keywords to use to find a reaction
        """
        ...
        # There are keywords
            ...
            response = await loop.run_in_executor(
                None, get, STRANGE_PLANET_SEARCH_API.format(quote(keywords), "reactions")
            )
            response = response.json()
