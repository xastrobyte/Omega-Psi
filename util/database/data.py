...
class Data:
    ...
    def set_session_user(self, user_id, session_data):
        ...
        """
        if not self.get_session_user(user_id):
            self._data.insert_one({"_id": user_id})
        self._data.update_one({"_id": user_id}, {"$set": session_data}, upsert = False)
    ...
    def get_insult_data_sync(self):
        ...
        if not insult_data:
            self._data.insert_one({"_id": "insults"})
            self.set_insult_data_sync(default)
            insult_data = default
        return insult_data
    ...
    def set_insult_data_sync(self, insult_data):
        ...
        ----------
            insult_data : dict
                The JSON object of the pending insults and insults
        """
        self._data.update_one({"_id": "insults"}, {"$set": insult_data}, upsert = False)
    ...
    def add_pending_insult_tags_sync(self, index, tags = []):
    ...
    def add_pending_compliment_tags_sync(self, index, tags = []):
